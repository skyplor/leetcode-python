...
class Solution:
    def maxPathSum(self, root: Optional[TreeNode]) -> int:
        ...
        def dfs(node: Optional[TreeNode]) -> int:
            ...
            max_left_child = max(0, dfs(node.left))
            max_right_child = max(0, dfs(node.right))

            res = max(res, node.val + max_left_child + max_right_child)
            return node.val + max(max_left_child, max_right_child)
