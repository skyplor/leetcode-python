...
class Router:
    ...
    def _remove_packet(self) -> List[int]:
        ...
        old_key = f'{old_source}_{old_destination}_{old_timestamp}'
        ...
        if old_destination_timestamps and old_destination_timestamps[0] <= old_timestamp:
            old_destination_timestamps.popleft()

        return [old_source, old_destination, old_timestamp]

    def addPacket(self, source: int, destination: int, timestamp: int) -> bool:
        key = f'{source}_{destination}_{timestamp}'
        ...
        if destination not in self.destination_timestamps:
            self.destination_timestamps[destination] = deque()
    ...
    def _search_timestamp_idx(self, timestamps: list[int], target: int, find_left: bool) -> int:
        '''
        Generic binary search that can find either left or right boundary
        find_left=True: finds leftmost position >= target
        find_left=False: finds rightmost position <= target
        '''
        if find_left:
    ...
    def getCount(self, destination: int, startTime: int, endTime: int) -> int:
        ...
        if not curr_destination_timestamps:
            return 0

        start_idx = self._search_timestamp_idx(
            curr_destination_timestamps, startTime, True)

        if start_idx == len(curr_destination_timestamps):
            return 0
        end_idx = self._search_timestamp_idx(
            curr_destination_timestamps, endTime, False)
